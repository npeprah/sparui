...
describe('PreloadScene Asset Loading Logic', () => {
  ...
  describe('Asset Loading Logic', () => {
    it('should plan to load exactly 34 card assets', () => {
    ...
    it('should generate unique asset keys for all cards', () => {
      const deck = generateSparDeck()
      const keys = new Set(
        deck.map((card) => `card_${card.suit}_${card.rank.toLowerCase()}`)
      )
      expect(keys.size).toBe(34) // All keys should be unique
    })
    ...
    it('should generate valid asset paths for all cards', () => {
      ...
      deck.forEach((card) => {
        ...
        expect(path).toMatch(/^\/assets\/cards\/(hearts|clubs|diamonds|spades)\/\1_[6-9]|10|[jqka]\.png$/)
      })
    })
  ...
  describe('Invalid Cards', () => {
    it('should not attempt to load 6 of spades', () => {
      const deck = generateSparDeck()
    ...
    it('should not attempt to load any cards with ranks 2-5', () => {
      const deck = generateSparDeck()
      const invalidRanks = deck.filter((card) =>
  ...
  describe('Total Asset Count', () => {
    it('should plan to load 34 cards + 1 card back = 35 assets total', () => {
      const deck = generateSparDeck()
      const totalAssets = deck.length + 1 // +1 for card back
      expect(totalAssets).toBe(35)
    })
  })
  ...
  describe('Surface Asset Loading', () => {
    ...
    it('should generate correct surface asset paths', () => {
      const expectedPaths = SURFACE_THEMES.map(
        theme => `assets/surfaces/surface_${theme}.png`
      )
